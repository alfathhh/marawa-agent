...
class Store:
    ...
    def admit_inbound(self, event_id: str, phone: str, body: str) -> str:
        row = self.connection.execute(
            ...
            RETURNING event_id
        """),
            {"event_id": event_id, "phone": phone, "body": body},
        ).first()
        return "pending" if row else "duplicate"

    def claim_inbound(self, claim_token: str, limit: int = 10):
        rows = (
            ...
            UPDATE inbound_events i SET status='PROCESSING', claim_token=:token,
                lease_until=now() + interval '90 seconds', attempts=i.attempts+1,
                updated_at=now()
            FROM candidates WHERE i.event_id=candidates.event_id
            RETURNING i.event_id, i.phone, i.body, i.claim_token, i.attempts
        """),
                {"token": claim_token, "limit": limit},
            )
            .mappings()
            .all()
        )
    ...
    def mark_inbound_done(self, event_id: str, claim_token: str):
        return (
            self.connection.execute(
                text("""
            UPDATE inbound_events SET status='DONE', claim_token=NULL, lease_until=NULL,
                updated_at=now() WHERE event_id=:id AND status='PROCESSING'
                AND claim_token=:token RETURNING event_id
        """),
                {"id": event_id, "token": claim_token},
            ).first()
            is not None
        )

    def mark_inbound_failed(
        self, event_id: str, claim_token: str, error: str, max_attempts: int = 8
    ):
        row = self.connection.execute(
            text("""
    ...
    def enqueue_outbound(self, phone: str, body: str, dedupe_key: str | None):
        row = self.connection.execute(
            ...
            RETURNING id
        """),
            {"phone": phone, "body": body, "dedupe_key": dedupe_key},
        ).first()
    ...
    def mark_failed(
        self, outbound_id: int, claim_token: str, error: str, max_attempts: int = 8
    ):
        row = self.connection.execute(
            text("""
